...
file.write = function(filepath, contents, options) {
  ...
  try {
    // If encoding is not explicitly null, convert from string to an encoded
    // buffer. If no encoding was specified, use the default.
...
file.read = function(filepath, options) {
  ...
  try {
    ...
    if (options.encoding !== null) {
      result = iconv.decode(result, options.encoding || file.defaultEncoding);
...
file.copy = function(srcpath, destpath, options) {
  ...
  var src = file.read(srcpath, readWriteOptions);
  if (process) {
    grunt.verbose.write('Processing source...');
    try {
      src = options.process(src);
      grunt.verbose.ok();
    } catch(e) {
      grunt.verbose.error();
  ...
  if (src === false) {
    ...
    file.write(destpath, src, readWriteOptions);
  }
};
